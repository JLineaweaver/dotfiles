local lsp_custom_opts_ok, lsp_custom_opts = pcall(require, "jlineaweaver.lsp.custom_server_opts")
if not lsp_custom_opts_ok then
	print("LSP: error when requiring custom options.")
	return
end

local lsp_default_handlers_status_ok, lsp_default_handlers = pcall(require, "jlineaweaver.lsp.default_handlers")
...
local completion_status_ok, _ = pcall(require, "jlineaweaver.completions")
